...
var getRemoteIndex = require("atomic-algolia/lib/utils/getRemoteIndex")
...
module.exports = function update(indexName, indexData, options, cb) {
    try {
        ...
        function runIndex() {
            getRemoteIndex(index)
                .then(function(oldIndex) {
                    // Figure out which records to add or delete
                    var operations = calculateOperations(newIndex, oldIndex)

                    if (options.verbose === true) {
                        console.log(`Algolia: Adding ${operations.add.length} hits to ${indexName}`)
                        console.log(`Algolia: Updating ${operations.update.length} hits to ${indexName}`)
                        console.log(`Algolia: Removing ${operations.delete.length} hits from ${indexName}`)
                        console.log(`Algolia: ${operations.ignore.length} hits unchanged in ${indexName}`)
                    }
